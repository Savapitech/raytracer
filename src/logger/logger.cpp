...
void Logger::log_info(Logger::LogLvl lvl, const std::string &msg) noexcept {
  if (lvl == Logger::NONE)
    return;
  if (lvl == Logger::INFO)
    std::cout << CLR_BOLD_INFO << "Info: " << CLR_INFO << msg << CLR_RESET
              << std::endl;
  if (lvl == Logger::WARNING)
    std::cout << CLR_BOLD_WARNING << "Warning: " << CLR_WARNING << msg
              << CLR_RESET << std::endl;
  if (lvl == Logger::ERROR)
    std::cout << CLR_BOLD_ERROR << "Error: " << CLR_ERROR << msg << CLR_RESET
              << std::endl;
  if (lvl == Logger::DEBUG)
    std::cout << CLR_BOLD_DEBUG << "Debug: " << CLR_DEBUG << msg << CLR_RESET
              << std::endl;
}
...
void Logger::SetLogLvl(const LogLvl lvl) noexcept { Logger::_loggerLvl = lvl; }
