...
class Parser // Classe Parser, sert à parser les arguments de la ligne de
             // commande.
{
public:
...
private:
  std::vector<std::string> _cmdArg; // std::vector de string CmdArg, contient
  ...
  void bindField(T &target,
                 const std::string &name); // méthode bindField, qui fill la
                                           // std::map ci-dessus.
