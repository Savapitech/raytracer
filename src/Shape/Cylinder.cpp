...
AABB Cylinder::getObjectAABB() const noexcept {
  ...
  return AABB({_pos.x - max, _pos.y - max, _pos.z - max},
              {_pos.x + max, _pos.y + max, _pos.z + max});
}

Vec3 Cylinder::getCentroid() const noexcept { return _pos; }

Vec2 Cylinder::getUv(Vec3 &) const noexcept { return {0, 0}; }
