...
AABB Cone::getObjectAABB() const noexcept {
  return AABB({_pos.x - _radius, _pos.y - _radius, _pos.z - _radius},
              {_pos.x + _radius, _pos.y + _radius, _pos.z + _radius});
}

Vec3 Cone::getCentroid() const noexcept { return _pos; }

Vec2 Cone::getUv(Vec3 &) const noexcept { return {0, 0}; }
