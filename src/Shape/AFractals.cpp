...
Vec3 AFractals::calculateNormale(const Vec3 &p) const 
{
    ...
    float dx = evaluateSDF(Vec3(p.x + epsilon, p.y, p.z)) - evaluateSDF(Vec3(p.x - epsilon, p.y, p.z));
    float dy = evaluateSDF(Vec3(p.x, p.y + epsilon, p.z)) - evaluateSDF(Vec3(p.x, p.y - epsilon, p.z));
    float dz = evaluateSDF(Vec3(p.x, p.y, p.z + epsilon)) - evaluateSDF(Vec3(p.x, p.y, p.z - epsilon));
