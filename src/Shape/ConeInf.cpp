...
{
    float inf = 1e6f;
    return AABB({_pos.x - inf, _pos.y - inf, _pos.z - inf}, {_pos.x + inf, _pos.y + inf, _pos.z + inf});
}

Vec3 ConeInf::getCentroid() const noexcept
{
    return _pos;
}

Vec2 ConeInf::getUv(Vec3 &) const noexcept
{
    return {0, 0};
}
