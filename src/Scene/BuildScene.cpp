...
void Scene::readObject(const Setting &s,
  ...
  for (int i = 0; i < count; i++) {
    ...
    Log::Logger::debug("Objects type: Shape:" +
                       objects.back()->getShape()->getType());
    if (objects.back()->getMaterial() != nullptr)
      Log::Logger::debug("Objects type: Material:" +
                         objects.back()->getMaterial()->type);
  }
  insertObjInObjects(s, objects);
}

void Scene::readLight(const Setting &s,
...
Scene::Scene(const std::string &scene_path) {
  ...
  if (scene.exists("background")) {
    Log::Logger::debug("Add Background" + (std::string)scene["background"] +
                       ":" +
                       std::to_string(Material::textureManager.uploadTexture(
                           (std::string)scene["background"])));
  }

  _cameraInfo = readcam(scene);
  readObject(scene, _objects);
  readLight(scene, _lights);
  // this->_lights.push_back(std::make_unique<AreaLight>(_cameraInfo.pos,
  // Vec3{1.0f, 1.0f, 1.0f}, 8));
  // this->_lights.push_back(std::make_unique<AreaLight>(Vec3{0, 80, 10},
  // Vec3{1.0f, 1.0f, 1.0f}, 8));
  // this->_lights.push_back(std::make_unique<AreaLight>(Vec3{30, 40, -20},
  // Vec3{1.0f, 1.0f, 1.0f}, 8));
  // this->_lights.push_back(std::make_unique<AreaLight>(Vec3{-30, 20, 30},
  // Vec3{10.00f, 10.0f, 10.00f}, 8));
}
} // namespace scene
