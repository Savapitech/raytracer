...
/*Update the left side and the right side of each element for estimate the cost
 * in the SAH*/
...
void BVH::appliedSah(bvh_stack_t &stack, int &outPivot, float &outBestCost,
                     float parentArea) {
  int count = stack.end - stack.start;
  ...
  for (int i = 0; i < count - 1; i++) {
    ...
    float cost = costTraversal +
                 costIntersect * (pLeft * leftCount + pRight * rightCount);
...
/*===Create every single node of the bvh with a simulate stack alloc on the
 * heaps===*/
void BVH::fillNode(std::vector<bvh_stack_t> &_myStacks) {
  ...
  if (newNode.count <= 2 ||
      (leafCost <= splitCost && newNode.count <= MAX_LEAF_SIZE)) {
    newNode.isLeaf = true;
    ...
    for (int i = 1; i < newNode.count; i++) {
      ...
      newNode.nodeShape = makeUnion(
          newNode.nodeShape, _objects[_indexTab[stack.start + i]]->getAABB());
    }
  } else {
...
void BVH::buildSpacePartitionning(void) {
  ...
  Log::Logger::debug("Three alloc:" +
                     std::to_string(THREE_ALLOC(_objects.size())));
  ...
  if (Log::Logger::GetLogLvl() == Log::Logger::LogLvl::DEBUG)
    for (size_t i = 0; i != _spacialThree.size(); i++)
      std::cout << CLR_BOLD_DEBUG << "Three BVH:" << CLR_DEBUG "Node: " << i
                << " " << "Is leaf:" << _spacialThree[i].isLeaf << " " << "Left"
                << _spacialThree[i].left << " " << "Right"
                << _spacialThree[i].right << " " << CLR_RESET << std::endl;
}
