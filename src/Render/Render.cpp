...
void Render::createRayBuffer(void) noexcept {
  ...
  Log::Logger::info("Rendering with " + std::to_string(numThreads) +
                    " threads");

  auto worker = [&]() {
    int tile;
    while ((tile = tileIdx.fetch_add(1, std::memory_order_relaxed)) <
           numTiles) {
  ...
  while (doneTiles.load(std::memory_order_acquire) < numTiles) {
    if (_gr->needsLiveUpdate()) {
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(DISPLAY_INTERVAL_MS));
    }
  }
...
void Render::renderSubset(const std::vector<uint32_t> &tileIndices, int tilesX,
                          int numThreads) noexcept {
  std::atomic<int> idx{0};
  int n = static_cast<int>(tileIndices.size());

  auto worker = [&]() {
    int i;
    while ((i = idx.fetch_add(1, std::memory_order_relaxed)) < n) {
      uint32_t t = tileIndices[static_cast<size_t>(i)];
      fillTile(static_cast<int>(t % static_cast<uint32_t>(tilesX)) * TILE_SIZE,
               static_cast<int>(t / static_cast<uint32_t>(tilesX)) * TILE_SIZE);
    }
  };
...
void Render::runRender(void) noexcept {
  ...
  while (_gr->isOpen()) {
    ...
    /*===Check the if we need to rebuild the frame buffer in case of
     * movement===*/
