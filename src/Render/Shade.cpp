...
Vec3 computeSpecular(const Vec3 &viewDir, const Vec3 &normal,
  ...
  Vec3 reflectDir =
      normalize(normal * (2.0f * dot(normal, lightDir)) - lightDir);
...
bool Render::launchShadowRay(const Vec3 &lightPos, const Vec3 &hitPoint,
                             const Vec3 &lightDir,
                             const Vec3 &normal) noexcept {
  Hit shadowHit;
  float epsilon =
      1e-4 * ((1.0f > hitPoint.length()) ? 1.0f : hitPoint.length());
  ...
  if (_bvh.intersect(shadowRay, shadowHit) == true) {
    const auto &hitMaterial =
        _scene.getObjects()[shadowHit.ObjectIdx]->getMaterial();
...
Vec3 computeRefraction(const Vec3 &incidentDir, const Vec3 &normal,
                       float refractionRatio) noexcept {
  ...
  Vec3 refractedParallel =
      normal * -std::sqrt(std::abs(1.0f - dot(refractedPerp, refractedPerp)));
  return refractedPerp + refractedParallel;
}

float computeNormalDistributionGGX(const Vec3 &normal, const Vec3 &halfVector,
...
float computeGeometrySmith(const Vec3 &normal, const Vec3 &viewDir,
                           const Vec3 &lightDir, float roughness) noexcept {
...
Vec3 computeFresnelSchlick(float cosTheta,
                           const Vec3 &baseReflectivity) noexcept {
  float clampedCos = std::clamp(1.0f - cosTheta, 0.0f, 1.0f);
  float c2 = clampedCos * clampedCos;
  float c4 = c2 * c2;
  return baseReflectivity +
         (Vec3(1.0f, 1.0f, 1.0f) - baseReflectivity) * (c4 * clampedCos);
}
...
Vec3 Render::applyPBR(Ray &ray, Hit &minHit, const Vec3 &albedoNorm) noexcept {
  ...
  Vec3 baseReflectivity =
      lerp(Vec3(0.04f, 0.04f, 0.04f), albedoNorm, material->metallic);
  ...
  if constexpr (IsPathTracer) {
    for (const auto &light : _scene.getLights()) {
      ...
      float normalDistrib =
          computeNormalDistributionGGX(normal, halfVector, material->roughness);
      float geometryTerm = computeGeometrySmith(normal, viewDir, centerLightDir,
                                                material->roughness);
      Vec3 fresnelTerm = computeFresnelSchlick(
          std::max(dot(halfVector, viewDir), 0.0f), baseReflectivity);

      Vec3 specularBRDF =
          (fresnelTerm * normalDistrib * geometryTerm) /
          std::max(4.0f * std::max(dot(normal, viewDir), 0.001f) * NdotL,
                   0.05f);

      Vec3 diffuseContrib =
          (Vec3(1.0f, 1.0f, 1.0f) - fresnelTerm) * (1.0f - material->metallic);
  ...
  if constexpr (!IsPathTracer) {
    for (const auto &light : _scene.getLights()) {
      ...
      for (int i = 0; i < numSamples; i++) {
        ...
        if (!launchShadowRay(jitteredLightPos, hitPoint, jitteredLightDir,
                             normal))
      ...
      totalRadiance += (diffuseContrib * (albedoNorm / PI) + specularBRDF) *
                       incomingRadiance * NdotL * shadowFactor;
    }
...
Vec3 Render::shade(Ray &ray, Hit &hit, int depth) noexcept {
  ...
  case TextureType::CHESSBOARD:
    albedo = ProceduralTexture::getChessboard(uv, material->color,
                                              material->colorChess, 2.f);
    break;
  case TextureType::LOAD_IMAGE:
    albedo =
        material->textureManager.getTexturePix(material->textureIndex, uv) *
        255;
    break;
  default:
  ...
  if constexpr (IsPathTracer == false) {
    ...
    if (material->transmission == 0.0f) {
      ...
      if (material->roughness < 0.3f) {
        ...
        baseReflectivity =
            lerp(baseReflectivity, albedoNorm, material->metallic);
        ...
        if (_bvh.intersect(reflectedRay, reflectHit)) {
          ...
          Vec2 hdriUv;
          hdriUv.x =
              0.5f + (std::atan2(reflectDir.z, reflectDir.x) / (2.0f * M_PI));
          hdriUv.y = 0.5f + (std::asin(reflectDir.y) / M_PI);
          finalColorNorm += Material::textureManager.getTexturePix(0, hdriUv) *
                            fresnelTerm * (1.0f - material->roughness);
        }
        Vec3 reflectionContrib =
            bounceColor * fresnelTerm * (1.0f - material->roughness);
      ...
      float refractionRatio =
          hit.frontFace ? (1.0f / material->ior) : material->ior;
      ...
      float schlickFresnel =
          schlickBase +
          (1.0f - schlickBase) * std::pow((1.0f - cosTheta), 5.0f);

      Vec3 reflectDir = reflect(unitIncidentDir, hit.normal);
      Vec3 refractDir =
          computeRefraction(unitIncidentDir, hit.normal, refractionRatio);
      ...
      if (totalInternalReflection) {
        ...
        finalGlassColor = reflectColor * schlickFresnel +
                          refractColor * (1.0f - schlickFresnel);
      }
    ...
    if (material->transmission > 0.0f) {
      ...
      if (cannotRefract || fastRandomFloat(0.0f, 1.0f) < reflectance) {
        ...
        bounceDir =
            normalize(reflected + randomUnitVector() * material->roughness);
        bounceOrigin = hit.position + hit.normal * 0.001f;
      } else {
        Vec3 refracted =
            computeRefraction(unitIncidentDir, hit.normal, refractionRatio);
        bounceDir =
            normalize(refracted + randomUnitVector() * material->roughness);
        bounceOrigin = hit.position - hit.normal * 0.001f;
      }
      ...
      Vec3 fresnelTerm = computeFresnelSchlick(cosTheta, baseReflectivity);
      float specProbability = std::clamp(
          (fresnelTerm.x + fresnelTerm.y + fresnelTerm.z) / 3.0f, 0.0f, 1.0f);
      ...
      if (fastRandomFloat(0.0f, 1.0f) < specProbability) {
        ...
        if (dot(bounceDir, hit.normal) < 0.0f) {
          bounceDir =
              bounceDir - hit.normal * dot(bounceDir, hit.normal) * 2.0f;
        }
        reflectionFilter =
            lerp(Vec3(1.0f, 1.0f, 1.0f), albedoNorm, material->metallic);
      } else {
      ...
      if (_bvh.intersect(bounceRay, bounceHit)) {
        ...
        Vec2 uv;
        uv.x = 0.5f +
               (std::atan2(bounceRay.dir.z, bounceRay.dir.x) / (2.0f * M_PI));
        uv.y = 0.5f + (std::asin(bounceRay.dir.y) / M_PI);
        indirectLighting =
            reflectionFilter * Material::textureManager.getTexturePix(0, uv);
      }
      finalColorNorm = directLighting + indirectLighting;
    }
  }
  if (finalColorNorm.x > 1)
    finalColorNorm.x = 1;
  if (finalColorNorm.y > 1)
    finalColorNorm.y = 1;
  if (finalColorNorm.z > 1)
    finalColorNorm.z = 1;
